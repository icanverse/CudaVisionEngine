...
std::string openFileDialog() {
    ...
    ofn.lpstrTitle = "Projeye Görsel Ekle";
...
void RightPanel::render(float displayWidth, float displayHeight) {
    ...
    if (isImageReadyForGPU) {
        ...
        if (rawResizedData) {
            textureID = TextureUtility::CreateTextureFromPixels(
                rawResizedData,
                256,
                144
            );
    ...
    // ==========================================
    // 3. OLUŞTUR BUTONU (KAYMA SORUNU ÇÖZÜLDÜ)
    // ==========================================
    float buttonHeight = 45.0f;
    float currentY = ImGui::GetCursorPosY();

    // YENİ MATEMATİK: Ekranın "görünen" alt sınırını Scroll (Kaydırma) miktarı ile birleştiriyoruz.
    float targetY = ImGui::GetWindowHeight() - buttonHeight - 15.0f + ImGui::GetScrollY();

    // Sadece gerçekten boşluk varsa butonu aşağı it.
    if (currentY < targetY) {
        ...
        if (!selectedImagePath.empty()) {
            ...
            std::thread([this, pathCopy]() {
                ...
                unsigned char* resizedPixels =
                    TextureUtility::LoadResizedPixels(
                        pathCopy,
                        256,
                        144,
                        originalWidth,
