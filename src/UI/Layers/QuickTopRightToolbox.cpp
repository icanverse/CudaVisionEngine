...
void QuickTopRightToolbox::render(float displayWidth, float displayHeight) {
    const float iconSizeXY = displayHeight / 32.0f;
    const float padding = 10.0f;
    const float spacing = 5.0f;
    const float buttonWidth = iconSizeXY + (ImGui::GetStyle().FramePadding.x * 2.0f);

    const float toolbarWidth = (padding * 2.0f) +
                               (availableTools.size() * buttonWidth) +
                               ((availableTools.size() - 1) * spacing);

    const float toolbarHeight = (padding * 2.0f) + iconSizeXY + (ImGui::GetStyle().FramePadding.y * 2.0f);

    const float xPos = displayWidth - toolbarWidth - 15.0f;
    const float yPos = 15.0f;
    ...
    ImGui::BeginChild("Sag Ust Arac Kutusu", ImVec2(toolbarWidth, toolbarHeight), false, toolboxFlags);
    ...
    for (size_t i = 0; i < availableTools.size(); ++i) {
        ...
        const ImVec2 iconSize(iconSizeXY, iconSizeXY);
